...
export default function DocumentRAGInterface() {
  ...
  return (
    <main className="flex h-[100dvh] min-h-0 flex-col overflow-hidden bg-zinc-950 text-zinc-100">
      <section className="shrink-0 border-b border-zinc-800 bg-zinc-950/95">
        <div className="mx-auto flex w-full max-w-[96rem] flex-col gap-3 px-4 py-3 sm:px-6 lg:px-8">
          <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
            <div className="min-w-0">
              <div className="mb-1.5 flex flex-wrap items-center gap-2 text-xs font-medium">
            ...
            <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-end">
              <div className="grid grid-cols-2 gap-2 text-xs sm:grid-cols-4 lg:w-[520px]">
                {CAPABILITIES.map((item) => (
                  <div
                    key={item.name}
                    className="flex min-h-12 items-center gap-2 rounded-md border border-zinc-800 bg-zinc-900/70 px-3 py-2"
                  >
                    <item.icon className="h-4 w-4 flex-shrink-0 text-cyan-300" />
                    <div className="min-w-0">
                      <div className="font-medium leading-4 text-white">{item.name}</div>
                      <div className="leading-4 text-emerald-300">{item.status}</div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {isDesktop && (
                <button
                  ...
                  disabled={importing}
                  className="inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-md border border-zinc-700 px-4 py-2 text-sm font-medium text-zinc-100 transition hover:border-cyan-500 hover:bg-zinc-900 disabled:cursor-not-allowed disabled:text-zinc-500 lg:w-auto"
                >
                  {importing ? (
          ...
          <nav
            className="-mx-4 flex gap-1 overflow-x-auto border-t border-zinc-900 px-4 pt-2 sm:mx-0 sm:px-0"
            aria-label="Views"
          >
            {VIEWS.map((item) => (
              <button
                ...
                className={`inline-flex min-h-9 shrink-0 items-center gap-2 rounded-md px-3 py-1.5 text-sm font-medium transition ${
                  view === item.id
      ...
      <section className="mx-auto flex min-h-0 w-full max-w-[96rem] flex-1 flex-col space-y-3 overflow-hidden px-4 py-3 sm:px-6 lg:px-8">
        {error && (
