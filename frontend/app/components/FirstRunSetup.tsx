...
export default function FirstRunSetup({
  ...
  const [generation, setGeneration] = useState<GenerationCapability | null>(null);
  ...
  const check = useCallback(async () => {
    ...
    setGeneration(readiness.data?.capabilities?.generation ?? null);
  ...
  const usesOllama = settings?.generation_provider === 'ollama';
  const canContinue = embeddingsReady && generationReady;
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-5">
      ...
      {/* Generation */}
      <div className={stepClass(!generationReady)}>
          ...
          <div>
            <h3 className="flex items-center gap-2 font-medium text-white">
              ...
              <span className="text-xs font-normal text-zinc-500">
                {settings?.generation_provider ?? 'local'}
              </span>
            </h3>
            <p className="mt-1 text-sm text-zinc-400">
              {checking
                ? 'Checking the generation backend…'
                : generationReady
                  ? `Ready — ${generation?.model}`
                  : usesOllama
                    ? 'Ollama is selected but is not ready.'
                    : 'Local GGUF model file is missing or cannot be loaded.'}
            </p>
          </div>
          <button
            type="button"
        ...
        {!checking && !generationReady && !usesOllama && (
          <p className="mt-3 rounded-md border border-zinc-800 bg-zinc-950 p-3 text-xs text-zinc-400">
            Put a GGUF model at{' '}
            <code className="text-zinc-300">{settings?.local_llm_model_path}</code>, or switch
            generation provider to Ollama in Settings.
          </p>
        )}
        {!checking && !generationReady && usesOllama && (
          <p className="mt-3 rounded-md border border-zinc-800 bg-zinc-950 p-3 text-xs text-zinc-400">
            Start Ollama with <code className="text-zinc-300">ollama serve</code>, then pull the
      ...
      {/* Step 2 — profile */}
      <div className={stepClass(usesOllama && ollamaReachable && missing.length > 0)}>
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2">
          {Object.entries(settings?.model_profiles ?? {}).map(([name, option]) => (
            <button
              key={name}
              type="button"
              disabled={!usesOllama || !ollamaReachable}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        >
          {canContinue ? 'Start using the app' : 'Waiting for local models'}
        </button>
      </div>

      {!generationReady && !checking && embeddingsReady && (
        <p className="flex items-start gap-2 text-xs text-zinc-500">
          <AlertCircle className="mt-0.5 h-3.5 w-3.5 flex-shrink-0" />
          You can still import and search documents once setup is skipped. Written answers stay
          unavailable until the selected generation backend is ready.
        </p>
      )}
    </div>
  );
