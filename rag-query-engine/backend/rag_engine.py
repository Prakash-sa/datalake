...
class DocumentRAGEngine:
    ...
    def index_documents(self, documents: List[dict]) -> dict:
        ...
        if len(documents) > self.MAX_DOCUMENTS:
            logger.warning(
                f"Document count {len(documents)} exceeds max {self.MAX_DOCUMENTS}"
            )
        ...
        try:
            ...
            logger.info(
                f"✅ Indexed {indexed_count} documents (failed: {failed_count})"
            )
...
Answer:"""
            ...
            try:
                ...
                response_text = (
                    f"Unable to generate response. Error: {str(llm_error)[:100]}"
                )
