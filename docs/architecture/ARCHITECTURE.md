...
Layered `api → application → domain`, with `infrastructure` supplying adapters
and dependencies pointing inward only. Its ingestion state machine, retrieval
pipeline, embedding providers, index fingerprints, and streaming protocol are
described in **[backend/ARCHITECTURE.md](BACKEND.md)** rather
than repeated here.

The parts that matter at system level:

- **Embeddings run in-process**, so importing and searching need no external
  software. Ollama is optional and used only to write answers.
- **Retrieval is hybrid** — dense vectors fused with SQLite FTS5 by reciprocal
  rank fusion — because neither retriever alone is sufficient.
- **The index records what built it**, so a changed embedding model is refused
  with a rebuild path rather than returning meaningless similarity scores.
- **Ingestion is a persisted queue**, so progress survives a restart and imports
