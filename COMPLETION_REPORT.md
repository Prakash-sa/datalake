...
- [x] Test pipeline script created (`scripts/test-pipeline.sh`)
- [x] Document upload & trigger script (`scripts/upload-and-trigger.sh`)
- [x] Live monitoring dashboard (`scripts/monitor-pipeline.sh`)
- [x] Live status tracker (`scripts/live-status.sh`)
- [x] Comprehensive pipeline guide (`PIPELINE_GUIDE.md`)
...
### Getting Help
1. Check `PIPELINE_GUIDE.md` for troubleshooting
2. Review logs: `docker logs <service_name>`
3. Check Airflow UI for task failures
4. Verify service health: `./scripts/test-pipeline.sh`
...
For detailed instructions, refer to `PIPELINE_GUIDE.md`.
